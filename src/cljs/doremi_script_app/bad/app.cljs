(ns doremi-script-app.app
  ...
  (:require 
    [doremi-script-app.utils :refer [my-log by-id log is-a my-log2] ]
...
(def log-off true)
(declare draw-item)
(declare expand_note_widths_to_accomodate_syllables)
(defn to-s[x]
...
(defn parse-results-box [{results :results }]
  [:div.form-group
   ...
   [:textarea#parse-results.form-control 
    {:rows "3" 
     ...
     :readOnly true
     :value 
     (.stringify js/JSON (clj->js results))
     }
    ]])
...
(defn composition[]
  (let [parsed (get-in @app-state [:parse-results,:parsed])]
  (my-log "***composition, parsed is")
  (my-log2 parsed)
  (if (nil? parsed)
...
(def notes-line-wrapper 
  (with-meta notes-line
             {:component-did-mount
              (fn[this]
                (expand_note_widths_to_accomodate_syllables (reagent/dom-node this)) 
...
(defn doremi-box[]
  ...
  [:div.doremiBox
   [controls]
    ...
    [composition]
   [staff-notation]
   [parse-results-box {:results (or
                                  (get-in @app-state [:parse-results,:parsed])
                                  (get-in @app-state [:parse-results,:error]))
...
(defn parse[]
  (let [
        elem (by-id "the_area")
        current (.-value elem)
       _ (my-log2 "current: " current)
        last-text-parsed (get @app-state :last-text-parsed)
       _ (my-log2 "last-text-parsed: " last-text-parsed)
